...
public class ReviewServiceImpl implements ReviewService {
    ...
    public Page<ReviewResponse> getProductReviewsWithFilters(Long productId, ReviewFilterRequest filters,
                                                             Pageable pageable) {
        log.debug("Fetching filtered reviews for product {}", productId);

        Page<Review> reviews = reviewRepository.findByFilters(
                productId,
                null, // userId not present in ReviewFilterRequest
                filters.getRating(),
    ...
    @Override
    @Transactional(readOnly = true)
    @Cacheable(value = "reviews", key = "'search:' + #productId + ':' + T(org.springframework.util.DigestUtils).md5DigestAsHex(#keyword.getBytes()) + ':' + #pageable.pageNumber + ':' + #pageable.pageSize + ':' + #pageable.sort")
    public Page<ReviewResponse> searchReviews(Long productId, String keyword, Pageable pageable) {
        if (keyword == null || keyword.trim().isEmpty()) {
            // Should call getProductReviews via injected dependency, but since this is a service, it's safe
    ...
    @Override
    @Transactional(readOnly = true)
    @Cacheable(value = "user-review-stats", key = "'user:' + #userId")
    ...
    @Override
    @Transactional(readOnly = true)
    @Cacheable(value = "rating-distribution", key = "'product:' + #productId")
    public Map<Integer, Map<String, Object>> getRatingDistribution(Long productId) {
        List<Object[]> results = reviewRepository.getRatingDistributionWithPercentages(productId);
        Map<Integer, Map<String, Object>> distribution = new HashMap<>();
    ...
    @Override
    @Transactional(readOnly = true)
    @Cacheable(value = "review-trends", key = "'product:' + #productId + ':months:' + #months")
    ...
    @Override
    @Transactional(readOnly = true)
    @Cacheable(value = "review-pros-cons", key = "'pros:' + #productId + ':' + #limit")
    public List<Map<String, Object>> getMostCommonPros(Long productId, int limit) {
        List<String> pros = reviewRepository.getMostCommonPros(productId, limit);
        return pros.stream()
                .map(pro -> {
    ...
    @Override
    @Transactional(readOnly = true)
    @Cacheable(value = "review-pros-cons", key = "'cons:' + #productId + ':' + #limit")
    public List<Map<String, Object>> getMostCommonCons(Long productId, int limit) {
        List<String> cons = reviewRepository.getMostCommonCons(productId, limit);
        return cons.stream()
                .map(con -> {
                    Map<String, Object> map = new HashMap<>();
                    map.put("text", con);
    ...
    @Override
    @Transactional(readOnly = true)
    @Cacheable(value = "top-rated-products", key = "#limit")
    ...
    @Override
    @Transactional(readOnly = true)
    @Cacheable(value = "most-reviewed-products", key = "#limit")
    ...
    @Override
    @Transactional(readOnly = true)
    @Cacheable(value = "admin-reviews", key = "'pending:' + #pageable.pageNumber + ':' + #pageable.pageSize + ':' + #pageable.sort")
    ...
    @Override
    @Transactional(readOnly = true)
    @Cacheable(value = "admin-reviews", key = "'flagged:' + #pageable.pageNumber + ':' + #pageable.pageSize + ':' + #pageable.sort")
    ...
    @Override
    @Transactional(readOnly = true)
    @Cacheable(value = "review-validation", key = "'user:' + #userId + ':product:' + #productId")
    ...
    @Override
    @Transactional(readOnly = true)
    @Cacheable(value = "purchase-validation", key = "'user:' + #userId + ':product:' + #productId")
