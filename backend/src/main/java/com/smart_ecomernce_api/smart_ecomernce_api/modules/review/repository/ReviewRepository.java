...
public interface ReviewRepository extends BaseRepository<Review, Long> {
         ...
         */
        Page<Review> findByRatingAndIsActiveTrue(Integer rating, Pageable pageable);
         ...
         */
        @Query("SELECT r FROM Review r WHERE r.isActive = true AND r.rating BETWEEN :minRating AND :maxRating")
         ...
         */
        Page<Review> findByVerifiedPurchaseTrueAndIsActiveTrue(Pageable pageable);

        /**
         * Find approved reviews
         */
        Page<Review> findByApprovedTrueAndIsActiveTrue(Pageable pageable);
         ...
         */
        Page<Review> findByCreatedAtBetweenAndIsActiveTrue(LocalDateTime startDate, LocalDateTime endDate,
                        Pageable pageable);
        ...
        /**
         * Find recent reviews with pagination
         */
        @Query("SELECT r FROM Review r WHERE r.isActive = true ORDER BY r.createdAt DESC")
        ...
        /**
         * Search reviews by product and text
         */
        ...
        @Query("SELECT r FROM Review r WHERE r.isActive = true AND r.product.id = :productId ORDER BY r.helpfulCount DESC")
        List<Review> findMostHelpfulReviews(@Param("productId") Long productId, @Param("limit") int limit);

        @Query("SELECT r FROM Review r WHERE r.isActive = true AND r.product.id = :productId ORDER BY r.createdAt DESC")
        List<Review> findRecentReviews(@Param("productId") Long productId, @Param("limit") int limit);

        @Query("SELECT r FROM Review r WHERE r.approved = false AND r.isActive = true")
        Page<Review> findPendingReviews(Pageable pageable);

        @Query("SELECT r FROM Review r WHERE r.isActive = true AND r.flagged = true")
        ...
        @Modifying
        @Query("UPDATE Review r SET r.verifiedPurchase = true WHERE r.product.id = :productId AND r.user.id IN (SELECT oi.order.user.id FROM OrderItem oi WHERE oi.product.id = :productId)")
        int updateVerificationStatusFromOrders(@Param("productId") Long productId);
        ...
        @Query("SELECT r.createdAt, COUNT(r) FROM Review r WHERE r.product.id = :productId GROUP BY r.createdAt ORDER BY r.createdAt")
        List<Map<String, Object>> getReviewTrendsOverTime(@Param("productId") Long productId, @Param("months") int months);

        @Query("SELECT p FROM Review r JOIN r.pros p WHERE r.product.id = :productId GROUP BY p ORDER BY COUNT(p) DESC")
        List<String> getMostCommonPros(@Param("productId") Long productId, @Param("limit") int limit);

        @Query("SELECT c FROM Review r JOIN r.cons c WHERE r.product.id = :productId GROUP BY c ORDER BY COUNT(c) DESC")
        List<String> getMostCommonCons(@Param("productId") Long productId, @Param("limit") int limit);

        @Query("SELECT r.product.id, AVG(r.rating) as avgRating FROM Review r WHERE r.isActive = true GROUP BY r.product.id ORDER BY avgRating DESC")
        List<Map<String, Object>> getTopRatedProducts(@Param("limit") int limit);

        @Query("SELECT r.product.id, COUNT(r) as reviewCount FROM Review r WHERE r.isActive = true GROUP BY r.product.id ORDER BY reviewCount DESC")
        List<Map<String, Object>> getMostReviewedProducts(@Param("limit") int limit);

        @Query("SELECT COUNT(r), AVG(r.rating) FROM Review r WHERE r.user.id = :userId")
        Map<String, Object> getUserReviewStats(@Param("userId") Long userId);
}
