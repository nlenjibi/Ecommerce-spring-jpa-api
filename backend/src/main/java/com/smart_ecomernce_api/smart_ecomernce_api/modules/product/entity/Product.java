...
@SelectBeforeUpdate // Check if update is needed
@Cacheable
...
public class Product extends BaseEntity {
    ...
    public boolean isLowStock() {
        if (!trackInventory)
            return false;
        return getAvailableQuantity() > 0 && getAvailableQuantity() <= lowStockThreshold;
    }
    ...
    public void updateRating(BigDecimal newRating) {
        if (this.ratingCount == 0) {
            ...
            BigDecimal totalRating = this.ratingAverage
                    .multiply(BigDecimal.valueOf(this.ratingCount))
                    .add(newRating);
            this.ratingCount++;
            this.ratingAverage = totalRating
