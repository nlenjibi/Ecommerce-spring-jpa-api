...
import com.smart_ecomernce_api.smart_ecomernce_api.common.predicate.ReviewPredicates;
...
public class ReviewController {
    ...
    @GetMapping
    @RequestValidation(requireAuth = false)
    @Operation(summary = "Get all reviews with advanced filtering")
    public ResponseEntity<ApiResponse<PaginatedResponse<ReviewResponse>>> getAllReviews(
        ...
        Page<ReviewResponse> reviews = reviewService.findReviewsWithPredicate(
                predicates.buildActiveOnly(), pageable);
    ...
    @GetMapping("/search")
    @RequestValidation(requireAuth = false)
    @Operation(summary = "Advanced review search")
    public ResponseEntity<ApiResponse<PaginatedResponse<ReviewResponse>>> searchReviews(
        ...
        Pageable pageable = PageRequest.of(page, size,
                Sort.by(Sort.Direction.fromString(direction), sortBy));
        return ResponseEntity.ok(ApiResponse.success("Reviews searched successfully",
                PaginatedResponse.from(reviewService.findReviewsWithPredicate(predicate, pageable))));
    }
