...
public final class OrderPredicates {
    ...
    private static final String F_USER_FIRST_NAME  = "user.firstName";
    private static final String F_USER_LAST_NAME   = "user.lastName";
    private static final String F_ORDER_NUMBER     = "orderNumber";
    private static final String F_TOTAL_AMOUNT     = "totalAmount";
    private static final String F_DISCOUNT_AMOUNT  = "discountAmount";
    private static final String F_CREATED_AT       = "createdAt";
    private static final String F_SHIPPING_ADDRESS = "shippingAddress";
    private static final String F_PAYMENT_METHOD   = "paymentMethod";
    ...
    /** Filter on {@code totalAmount BETWEEN min AND max}. */
    public OrderPredicates withTotalAmountBetween(BigDecimal minAmount, BigDecimal maxAmount) {
        if (minAmount != null && maxAmount != null) {
            bb.and(Expressions.numberPath(BigDecimal.class, F_TOTAL_AMOUNT)
    ...
    /** Filter on {@code createdAt BETWEEN startDate AND endDate}. */
    public OrderPredicates withCreatedBetween(LocalDateTime startDate, LocalDateTime endDate) {
        if (startDate != null && endDate != null) {
            bb.and(Expressions.dateTimePath(LocalDateTime.class, F_CREATED_AT)
                    .between(startDate, endDate));
        }
        return this;
    }

    /** Case-insensitive contains on {@code shippingAddress}. */
    public OrderPredicates withShippingAddressContaining(String address) {
        if (hasText(address)) {
            bb.and(Expressions.stringPath(F_SHIPPING_ADDRESS).containsIgnoreCase(address));
        }
        return this;
    }
    ...
    /** Case-insensitive contains on {@code user.firstName} OR {@code user.lastName}. */
    public OrderPredicates withCustomerNameContaining(String name) {
        if (hasText(name)) {
            bb.and(
                    Expressions.stringPath(F_USER_FIRST_NAME).containsIgnoreCase(name)
                            .or(Expressions.stringPath(F_USER_LAST_NAME).containsIgnoreCase(name))
            );
        }
        return this;
    }

    public OrderPredicates withCompletedStatus() {
        // Use DELIVERED as the completed status, since OrderStatus.COMPLETED does not exist
        return withStatus(OrderStatus.DELIVERED);
    }

    /** Shortcut — restrict to {@link OrderStatus#PENDING}. */
    public OrderPredicates withPendingStatus() {
        return withStatus(OrderStatus.PENDING);
    }

    /** Shortcut — restrict to {@link OrderStatus#CANCELLED}. */
    public OrderPredicates withCancelledStatus() {
    ...
    /** Orders that have a non-zero {@code discountAmount}. */
    public OrderPredicates withDiscount() {
        bb.and(Expressions.numberPath(BigDecimal.class, F_DISCOUNT_AMOUNT).gt(BigDecimal.ZERO));
        return this;
    }

    /**
     * Filter by exact {@code paymentMethod} string value.
     * The value should match the enum name stored in the DB column (e.g. "CREDIT_CARD").
     */
    public OrderPredicates withPaymentMethod(String paymentMethod) {
        if (hasText(paymentMethod)) {
            bb.and(Expressions.stringPath(F_PAYMENT_METHOD).eq(paymentMethod));
        }
        return this;
    }
    ...
    /**
     * Abandoned carts: status is PENDING, created before {@code cutoffDate}.
     */
    public OrderPredicates withAbandonedBefore(LocalDateTime cutoffDate) {
        if (cutoffDate != null) {
            bb.and(
                    Expressions.enumPath(OrderStatus.class, F_STATUS).eq(OrderStatus.PENDING)
                            .and(Expressions.dateTimePath(LocalDateTime.class, F_CREATED_AT)
                                    .loe(cutoffDate))
            );
        }
        return this;
    ...
    /**
     * Filter orders with specific carrier
     */
    public OrderPredicates withCarrier(String carrier) {
        if (hasText(carrier)) {
            bb.and(Expressions.stringPath("carrier").containsIgnoreCase(carrier));
        }
        return this;
    }
